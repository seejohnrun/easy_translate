  ...
  describe klass do
    ...
    describe :options do
      ...
      it 'should accept timeouts options' do
        request = EasyTranslate::Detection::DetectionRequest.new "test", :http => {:timeout => 1, :open_timeout => 2}
      ...
      it 'should accept ssl options' do
        request = EasyTranslate::Detection::DetectionRequest.new "test", :http => {:ssl => {:verify_depth => 3, :ca_file => 'path/to/ca/file'}}
        http = request.send(:http)
